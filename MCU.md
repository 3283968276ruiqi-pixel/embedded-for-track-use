...
寄存器：
- 通用寄存器：存放运算数据、地址指针
- 特殊寄存器：栈指针、链接寄存器、程序计数器
- 状态寄存器：保存运算结果标志、中断状态
- 外设寄存器：控制外设工作模式（CPU只能直接操作寄存器中的数据，外设寄存器位于外设内部，通过总线被CPU访问）

总线：（MCU采用分层总线架构）
- 地址总线：CPU发出目标地址
- 数据总线：传输实际数据（32位、16位或8位）
- 控制总线：读写方向、传输宽度、等待信号等

外设：外设通过*内存映射*的方式被CPU访问

中断：中断是链接外设与CPU的异步通知机制

中断处理中的寄存器协作：
- 外设寄存器：产生中断请求（置位标志位）
- NVIC寄存器：控制中断技能、优先级、挂起状态
- CPU寄存器：自动保存R0--R3,R12,LR,PC,xPSR
- 总线：CPU通过总线读取向量表地址，跳转到ISR
